...
for `transition A, transition B, added A`, enumeration is `B, A`, while EDK2
reclaim output is `A, B`. At runtime, an append that does not fit returns
out-of-resources rather than planning reclaim.
...
NEXT cursor and end distinctions, QUERY accounting, append without scratch,
