...
run_case()
{
	...
	for tick in $(seq 1 20); do
		if test -f "$out/$name.serial" &&
			rg -q 'Q35 DMA: (handoff generation|default-deny enable failed|expected exactly one)' \
...
run_case protected -device intel-iommu,pt=off -device edu,dma_mask=0xffffffff
run_case missing_iommu -device edu,dma_mask=0xffffffff
run_case missing_requester -device intel-iommu,pt=off
run_case duplicate -device intel-iommu,pt=off \
...
rg -q 'Q35 DMA: default-deny active, EDU 0000:00:03.0 BME clear' \
	"$out/protected.serial"
