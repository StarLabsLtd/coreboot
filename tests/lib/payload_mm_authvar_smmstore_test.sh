...
mutant()
{
	...
	sed "$expression" "$root/src/lib/payload_mm_authvar_smmstore.c" > "$source"
	! cmp -s "$source" "$root/src/lib/payload_mm_authvar_smmstore.c"
	"${CC:-cc}" -std=gnu11 -Wall -Wextra -Werror -Wconversion -Wshadow \
...
mutant omit-post-program-context \
	'/result = spi_flash_volatile_lease_write/,/return PAYLOAD_MM_AUTHVAR_MEDIA_SUCCESS;/ s/if (result || !context_valid(opaque))/if (result)/' \
	callback-mutation
mutant omit-begin-cleanup \
	's/(void)spi_flash_volatile_lease_end(\&backend[.]lease);/(void)0;/' \
	begin-mutation
...
# shellcheck disable=SC2086
"${CC:-cc}" $common_smm_flags -c \
	"$root/src/lib/payload_mm_authvar_smmstore.c" \
	-o "$temporary/authvar_smmstore.smm.o"
ld -m elf_i386 -r "$temporary/read_region.smm.o" \
	"$temporary/authvar_smmstore.smm.o" -o "$temporary/authvar_backend.smm.o"
file "$temporary/authvar_backend.smm.o" | grep -Eq 'ELF 32-bit.*Intel (80386|i386)'
nm "$temporary/authvar_backend.smm.o" | \
	grep -q ' T payload_mm_authvar_smmstore_install$'
nm "$temporary/authvar_backend.smm.o" | \
...
grep -Fqx 'smm-$(CONFIG_PAYLOAD_MM_AUTHVAR_SMMSTORE_BACKEND) += payload_mm_authvar_smmstore.c' \
	"$root/src/lib/Makefile.mk"

printf '%s\n' 'Payload-MM authenticated-variable SMMSTORE backend: PASS'
