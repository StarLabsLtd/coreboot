...
#define BACKEND_SOURCE_INCLUDE "../../src/lib/payload_mm_authvar_smmstore.c"
#endif
...
int spi_flash_volatile_lease_begin(const struct spi_flash *active_flash,
...
{
	record(OP_READ);
	CHECK(active_flash == &flash && lease_owned && lease->private_data[0]);
...
int spi_flash_volatile_lease_write(const struct spi_flash *active_flash,
	const struct spi_flash_volatile_lease *lease, uint32_t offset, size_t size,
	const void *buffer)
{
	record(OP_WRITE);
...
int spi_flash_volatile_lease_erase(const struct spi_flash *active_flash,
	const struct spi_flash_volatile_lease *lease, uint32_t offset, size_t size)
{
	record(OP_ERASE);
	CHECK(active_flash == &flash && lease_owned && lease->private_data[0]);
...
int spi_flash_volatile_lease_sync(const struct spi_flash *active_flash,
...
static void install(void)
{
	CHECK(payload_mm_authvar_smmstore_install() == CB_SUCCESS);
...
static void test_geometry(const char *mode)
{
	if (!strcmp(mode, "geometry-offset"))
		...
		__builtin_trap();
	CHECK(payload_mm_authvar_smmstore_install() == CB_ERR);
}
