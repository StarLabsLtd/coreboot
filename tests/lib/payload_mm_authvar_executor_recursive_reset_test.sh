...
for optimization in ${RECURSIVE_OPTIMIZATIONS:-0 2}; do
	...
	0|2) ;;
	*)
		printf 'invalid recursive optimization: %s\n' "$optimization" >&2
		exit 1
		;;
	esac
	start="$(date +%s)"
	cc -std=gnu11 -O"$optimization" -Wall -Wextra -Werror -Wconversion \
		...
		"$root/tests/lib/payload_mm_authvar_executor_recursive_reset_test.c" \
	...
	printf 'recursive O%s runtime=%ss\n' "$optimization" "$((end - start))"
done

printf '%s\n' 'Payload-MM authenticated-variable recursive recovery resets: PASS'
