...
static uint16_t unlisted_command;
static int stuck;
...
{
	uint16_t value = device == &fifth_endpoint ? unlisted_command : command;

	return value | ((stuck || device == stuck_device) ? PCI_COMMAND_MASTER : 0);
}
...
void payload_resource_write_command(const struct device *device, uint16_t value)
{
	...
	if (device == &fifth_endpoint)
		unlisted_command = value;
	else
		command = value;
}
...
static void reset_fixture(void)
{
	...
	unlisted_command = PCI_COMMAND_MASTER | PCI_COMMAND_MEMORY;
	stuck = 0;
	stuck_device = NULL;
...
#define REJECT_FRAMEBUFFER(mutation, message) do { \
	...
	/* An enabled function without an assigned BAR is absent from rev4
	 * topology, but must still have BME cleared and verified. */
	reset_fixture();
	revision4_ready = 1;
	add_q35_boot_controllers();
	add_q35_lpc_logical_child();
	failures += check(lb_add_payload_resource_handoff(header) == CB_SUCCESS &&
		((const struct lb_payload_resource_handoff *)(const void *)
		 (storage + sizeof(*header)))->sections[2].entry_count == 4 &&
		!(unlisted_command & PCI_COMMAND_MASTER),
		"unlisted enabled PCI function retained bus mastering");
	reset_fixture();
	revision4_ready = 1;
	add_q35_boot_controllers();
	add_q35_lpc_logical_child();
	stuck_device = &fifth_endpoint;
	failures += check(lb_add_payload_resource_handoff(header) == CB_ERR &&
		header->table_entries == 0,
		"unlisted enabled PCI function with stuck BME was published");
...
#undef REJECT_FRAMEBUFFER
	...
	failures += check(!payload_resource_revision4_published() &&
		payload_resource_revision4_generation() == 0,
		"failed publication retained a revision 4 generation");
