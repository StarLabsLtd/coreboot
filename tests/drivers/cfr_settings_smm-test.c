...
static void test_reject_invalid_stored_value(void **state)
{
	store_value = 2;
	request(1, CFR_SETTINGS_CMD_GET, 0, 0);
	assert_int_equal(mailbox.status, CFR_SETTINGS_STATUS_STORAGE_ERROR);
	assert_false(mailbox.response_flags & CFR_SETTINGS_RESP_CURRENT_VALID);
	assert_int_equal(write_calls, 0);
	assert_int_equal(apply_calls, 0);
}
...
int main(void)
{
	const struct CMUnitTest tests[] = {
		...
		cmocka_unit_test_setup(test_reject_invalid_stored_value, setup),
