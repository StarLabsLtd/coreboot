...
static const struct sm_object touchpad_haptics = SM_DECLARE_ENUM({
	.flags		= !CONFIG(PAYLOAD_MM_INTERFACE) || CONFIG(STARLABS_TOUCHPAD_ACPI_RUNTIME) ?
	...
	.runtime_apply = {
		.method	= CONFIG(STARLABS_TOUCHPAD_ACPI_RUNTIME) ? CFR_RUNTIME_APPLY_ACPI :
			  CONFIG(PAYLOAD_MM_INTERFACE) ? CFR_RUNTIME_APPLY_NONE :
			  CFR_RUNTIME_APPLY_APM_CNT,
