...
void lb_board(struct lb_header *header)
{
	...
	const struct sm_object general_options[] = {
		SM_DECLARE_ENUM(enum_opts[2]),		// fan_mode
		SM_DECLARE_ENUM(enum_opts[4]),		// max_charge
		SM_DECLARE_ENUM(enum_opts[3]),		// kbl_timeout
		SM_DECLARE_BOOL(bool_opts[0]),		// fn_ctrl_swap
	#if CONFIG(EC_STARLABS_CHARGING_SPEED)
		SM_DECLARE_ENUM(enum_opts[7]),		// charging_speed
	#endif
	#if CONFIG(EC_STARLABS_LID_SWITCH)
		SM_DECLARE_ENUM(enum_opts[8]),		// lid_switch
	#endif
	};
	...
	const struct sm_object device_options[] = {
		...
		SM_DECLARE_BOOL(bool_opts[4]),		// thunderbolt
	#endif
	#if CONFIG(BOARD_STARLABS_STARBOOK_ADL)
		SM_DECLARE_BOOL(bool_opts[3]),		// pci_hot_plug
	#endif
		...
		SM_DECLARE_BOOL(bool_opts[9]),		// fingerprint_reader
	#endif
		SM_DECLARE_BOOL(bool_opts[10]),		// card_reader
	};

	/* Chipset Options */
	const struct sm_object chipset_options[] = {
		SM_DECLARE_ENUM(enum_opts[6]),		// power_profile
		SM_DECLARE_ENUM(enum_opts[5]),		// me_state
		SM_DECLARE_NUMBER(number_opts[0]),	// me_state_counter
		SM_DECLARE_BOOL(bool_opts[1]),		// hyper_threading
