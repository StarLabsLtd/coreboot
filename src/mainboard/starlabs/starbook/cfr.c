...
void lb_board(struct lb_header *header)
{
	...
	struct sm_obj_bool card_reader = {
		.object_id	= get_object_id(),
	...
	struct sm_obj_bool fingerprint_reader = {
		.object_id	= get_object_id(),
	...
	struct sm_obj_bool pci_hot_plug = {
		.object_id	= get_object_id(),
		.opt_name	= "pci_hot_plug",
		.ui_name	= "Third-Party SSD Support",
		.ui_helptext	= "Enables PCI Hot Plug, which slows down the SSD initialization. It is required for certain third-party SSDs to be detected.",
		.default_value	= false,
	};
	#endif
