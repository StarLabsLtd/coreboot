/* SPDX-License-Identifier: GPL-2.0-only */

#include <boot/coreboot_tables.h>
#include <commonlib/coreboot_tables.h>
#include <drivers/option/cfr.h>
#include <inttypes.h>
#include <string.h>
#include <types.h>
#include <variants.h>

static uint32_t starbook_get_option_id(void)
{
	static uint32_t option_id = 0;

	/* Let's start at 1: this way, option ID being 0 indicates someone messed up */
	return ++option_id;
}

static void write_main_form(struct lb_header *header)
{
	const struct sm_option_enum enum_opts[] = {
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "boot_option",
			.ui_name	= "Boot Option",
			.ui_helptext	= "Change the boot device in the event of a failed boot",
			.default_value	= 0,
			.values		= (const struct sm_enum_value[]) {
				{	"Fallback",	0		},
				{	"Normal",	1		},
				SM_ENUM_VALUE_END,
			}
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "debug_level",
			.ui_name	= "Debug Level",
			.ui_helptext	= "Set the verbosity of the debug output.",
			.default_value	= 0,
			.values		= (const struct sm_enum_value[]) {
				{	"Emergency",	0		},
				{	"Alert",	1		},
				{	"Critical",	2		},
				{	"Error",	3		},
				{	"Warning",	4		},
				{	"Notice",	5		},
				{	"Info",		6		},
				{	"Debug",	7		},
				{	"Spew",		8		},
				SM_ENUM_VALUE_END,
			}
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "fan_mode",
			.ui_name	= "Fan Mode",
			.ui_helptext	= "Adjust the fan curve to priotise performance or noise levels.",
			.default_value	= 1,
			.values		= (const struct sm_enum_value[]) {
				{	"Quiet",	0		},
				{	"Normal",	1		},
				{	"Aggressive",	2		},
				SM_ENUM_VALUE_END,
			}
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "kbl_timeout",
			.ui_name	= "Keyboard Backlight Timeout",
			.ui_helptext	= "Set the amount of time before the keyboard backlight turns off when un-used",
			.default_value	= 0,
			.values		= (const struct sm_enum_value[]) {
				{	"30 seconds",	0		},
				{	"1 minute",	1		},
				{	"3 minutes",	2		},
				{	"5 minutes",	3		},
				{	"Never",	4		},
				SM_ENUM_VALUE_END,
			}
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "max_charge",
			.ui_name	= "Maximum Charge Level",
			.ui_helptext	= "Set the maximum level the battery will charge to.",
			.default_value	= 0,
			.values		= (const struct sm_enum_value[]) {
				{	"100%",		0		},
				{	"80%",		1		},
				{	"60%",		2		},
				SM_ENUM_VALUE_END,
			}
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "me_state",
			.ui_name	= "Intel Management Engine",
			.ui_helptext	= "Enable or disable the Intel Management Engine",
			.default_value	= 1,
			.values		= (const struct sm_enum_value[]) {
				{	"Disabled",	1		},
				{	"Enabled",	0		},
			}
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "power_on_after_fail",
			.ui_name	= "Power On After Fail",
			.ui_helptext	= "Select whether to power on in the event of a power failure.",
			.default_value	= 0,
			.values		= (const struct sm_enum_value[]) {
				{	"Disable",	0		},
				{	"Enable",	1		},
				{	"Keep",		2		},
				SM_ENUM_VALUE_END,
			}
		},
		{

			.option_id	= starbook_get_option_id(),
			.opt_name	= "power_profile",
			.ui_name	= "Power Profile",
			.ui_helptext	= "Select whether to maximise performance, battery life or both.",
			.default_value	= 1,
			.values		= (const struct sm_enum_value []) {
				{	"Power Saver",	PP_POWER_SAVER	},
				{	"Balanced",	PP_BALANCED	},
				{	"Performance",	PP_PERFORMANCE	},
				SM_ENUM_VALUE_END,
			},
		},
	};

	const struct sm_option_bool bool_opts[] = {
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "fn_ctrl_swap",
			.ui_name	= "Fn Ctrl Reverse",
			.ui_helptext	= "Swap the functions of the [Fn] and [Ctrl] keys",
			.default_value	= false,
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "legacy_8254_timer",
			.ui_name	= "Legacy 8254 Timer",
			.ui_helptext	= "Enable or disable the legacy 8254 timer. Increases power consumption"
					  "by disabling clock gating, but increases compatibility with older"
					  "Operating Systems",
			.default_value	= false,
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "hyper_threading",
			.ui_name	= "Hyper-Threading",
			.ui_helptext	= "Enable or disable Hyper-Threading",
			.default_value	= true,
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "microphone",
			.ui_name	= "Microphone",
			.ui_helptext	= "Enable or disable the built-in microphone",
			.default_value	= true,
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "mirror_flag",
			.ui_name	= "Mirror Flag",
			.ui_helptext	= "Use the mirror flag to automatically update the EC firmware from coreboot.",
			.default_value	= false,
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "pci_hot_plug",
			.ui_name	= "Third-Party SSD Support",
			.ui_helptext	= "Enables PCI Hot Plug, which slows down the SSD initialisation. It is"
					  "required for certain third-party SSDs to be detected.",
			.default_value	= false,
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "thunderbolt",
			.ui_name	= "Thunderbolt",
			.ui_helptext	= "Enable or disable Thunderbolt support",
			.default_value	= false,
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "vtd",
			.ui_name	= "VT-d",
			.ui_helptext	= "Enable or disable Intel VT-d (virtualisation)",
			.default_value	= true,
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "webcam",
			.ui_name	= "Webcam",
			.ui_helptext	= "Enable or disable the built-in webcam",
			.default_value	= true,
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "wireless",
			.ui_name	= "Wireless",
			.ui_helptext	= "Enable or disable the built-in wireless card",
			.default_value	= true,
		},
	};

	const struct sm_option_number number_opts[] = {
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "kbl_brightness",
			.ui_name	= "Keyboard Backlight Brightness",
			.flags		= CFR_OPTFLAG_SUPPRESS,
			.default_value	= 0,
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "kbl_state",
			.ui_name	= "Keyboard Backlight State",
			.flags		= CFR_OPTFLAG_SUPPRESS,
			.default_value	= 0,
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "fn_lock_state",
			.ui_name	= "Fn Lock State",
			.flags		= CFR_OPTFLAG_SUPPRESS,
			.default_value	= 1,
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "me_state_counter",
			.ui_name	= "ME State Counter",
			.flags		= CFR_OPTFLAG_SUPPRESS,
			.default_value	= 0,
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "mirror_flag_counter",
			.ui_name	= "Mirror Flag Counter",
			.flags		= CFR_OPTFLAG_SUPPRESS,
			.default_value	= 0,
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "ramtop",
			.ui_name	= "Ramtop",
			.flags		= CFR_OPTFLAG_SUPPRESS,
			.default_value	= 0,
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "reboot_counter",
			.ui_name	= "Reboot Counter",
			.flags		= CFR_OPTFLAG_SUPPRESS,
			.default_value	= 0,
		},
		{
			.option_id	= starbook_get_option_id(),
			.opt_name	= "trackpad_state",
			.ui_name	= "Trackpad State",
			.flags		= CFR_OPTFLAG_SUPPRESS,
			.default_value	= 0,
		},
	};

	for (size_t i = 0; i < ARRAY_SIZE(number_opts); i++) {
		sm_write_option_number(header, &number_opts[i]);
	}

	for (size_t i = 0; i < ARRAY_SIZE(enum_opts); i++) {
		sm_write_option_enum(header, &enum_opts[i]);
	}

	for (size_t i = 0; i < ARRAY_SIZE(bool_opts); i++) {
		sm_write_option_bool(header, &bool_opts[i]);
	}

}

static void write_forms(struct lb_header *header)
{
	sm_write_form(header, "Main", write_main_form);
}

void lb_board(struct lb_header *header)
{
	sm_write_cfr(header, write_forms);
}
