...
static void configure_power_limits(config_t *cfg)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(cfg->power_limits_config); i++) {
		struct soc_power_limits_config *soc_power = &cfg->power_limits_config[i];

		/* Sset PL4 to 1.0C */
		soc_power->tdp_pl4 = 65;

		/* Set PL1 to 50% of PL2 */
		soc_power->tdp_pl1_override = (soc_power->tdp_pl2_override / 2) & ~1;

		/* Scale PL1 & PL2 based on CMOS settings */
		soc_power->tdp_pl1_override = (soc_power->tdp_pl1_override * performance_scale) / 100;
		soc_power->tdp_pl2_override = (soc_power->tdp_pl2_override * performance_scale) / 100;
