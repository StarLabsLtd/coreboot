...
//			Store (0x03, \_SB.PCI0.GFX0.CLID)
		...
		Method(_REG, 2, NotSerialized)
		{
			If (LAnd (LEqual (Arg0, 0x03), LEqual (Arg1, 0x01)))
			{
				...
				Store (0x00, BNUM)
				Or (BNUM, ShiftRight (And (ECRD (RefOf (ECWR)), 0x02), 0x01), BNUM)
			...
			Store (0x01, \_SB.PCI0.LPCB.H_EC.ECOS)
		}
		...
		{
			Store (Acquire (ECMT, 1000), Local0)
			If ((Local0 == Zero))
			{
				If (ECAV)
				{
		...
		Method (ECRD, 1, Serialized, 0, IntObj, FieldUnitObj)
		{
			...
			If ((Local0 == Zero))
			{
				Store ("----> ECRD: Acquired mutex", Debug)
				If (ECAV)
				{
					Store ("------> ECRD: ECAV set", Debug)
					// Execute read from Embedded Controller
					Store (DerefOf (Arg0), Local1)
					Release (ECMT)
					Store ("<----- ECRD: jumping out", Debug)
					Return (Local1)
				}
				Else
				{
					Store ("------> ECRD: ECAV not set", Debug)
					Release (ECMT)
				}
			}
			Store ("<---- ECRD: Exit", Debug)
			Return (Local1)
		}
