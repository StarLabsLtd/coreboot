...
static uint32_t sm_write_runtime_apply(char *current, uint32_t method, uint32_t id)
{
	struct lb_cfr_runtime_apply *runtime_apply = (struct lb_cfr_runtime_apply *)current;

	if (method == CFR_RUNTIME_APPLY_NONE)
		return 0;

	runtime_apply->tag = CFR_TAG_RUNTIME_APPLY;
	runtime_apply->size = sizeof(*runtime_apply);
	runtime_apply->method = method;
	runtime_apply->id = id;
...
static uint32_t write_numeric_option(char *current, uint32_t tag, const uint64_t object_id,
		...
		uint32_t runtime_apply_method, uint32_t runtime_apply_id,
		...
		const uint32_t num_dep_values)
{
	...
	current += sm_write_runtime_apply(current, runtime_apply_method, runtime_apply_id);
...

{
	return write_numeric_option(current, CFR_TAG_OPTION_ENUM, object_id,
			...
			sm_enum->runtime_apply_method, sm_enum->runtime_apply_id, dep_id,
...

{
	return write_numeric_option(current, CFR_TAG_OPTION_NUMBER, object_id,
			...
			sm_number->runtime_apply_method, sm_number->runtime_apply_id,
...

{
	return write_numeric_option(current, CFR_TAG_OPTION_BOOL, object_id,
			...
			sm_bool->runtime_apply_method, sm_bool->runtime_apply_id,
			dep_id, dep_values, num_dep_values);
}
