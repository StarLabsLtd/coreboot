...
static uint32_t sm_write_runtime_apply(char *current, struct sm_runtime_apply sm_runtime_apply)
{
	...
	if (sm_runtime_apply.method == CFR_RUNTIME_APPLY_NONE)
		return 0;
	if (sm_runtime_apply.method == CFR_RUNTIME_APPLY_APM_CNT &&
	    sm_runtime_apply.id > UINT8_MAX) {
