...
enum tree_dependency_state {
	TREE_DEPENDENCY_UNSEEN,
	TREE_DEPENDENCY_VISITING,
	TREE_DEPENDENCY_DONE,
};

/* SMM dispatch serializes access to this non-reentrant validation workspace. */
static struct tree_walk_scratch tree_scratch;
static const struct sm_object *tree_objects[CFR_SETTINGS_MAX_TREE_OBJECTS];
static uint8_t tree_dependency_states[CFR_SETTINGS_MAX_TREE_OBJECTS];
static uint16_t tree_stack_object[CFR_SETTINGS_MAX_TREE_OBJECTS];
static uint16_t tree_stack_next[CFR_SETTINGS_MAX_TREE_OBJECTS];
static size_t num_tree_objects;
...
static const struct sm_object *const *validated_forms;
static size_t validated_form_entries;
...
void cfr_settings_policy_reset_for_test(void)
{
	...
	validated_forms = NULL;
	validated_form_entries = 0;
}
#endif
...
static const char *option_ui_name(const struct sm_object *option)
{
	switch (option->kind) {
	case SM_OBJ_ENUM:
		return option->sm_enum.ui_name;
	case SM_OBJ_NUMBER:
		return option->sm_number.ui_name;
	case SM_OBJ_BOOL:
		return option->sm_bool.ui_name;
	default:
		return NULL;
	}
}
...
static bool enum_is_well_formed(const struct sm_object *option)
{
	...
	for (size_t i = 0; i < CFR_SETTINGS_MAX_ENUM_VALUES; i++) {
		if (!values[i].ui_name)
			return i != 0;
		if (!values[i].ui_name[0])
			return false;

		for (size_t j = 0; j < i; j++)
			if (values[j].value == values[i].value ||
			    !strcmp(values[j].ui_name, values[i].ui_name))
				return false;
	}
...
static bool numeric_option_is_well_formed(const struct sm_object *option)
{
	const char *name;
	const char *ui_name;

	if (!option || option->ctor ||
	    (option->kind != SM_OBJ_BOOL && option->kind != SM_OBJ_ENUM &&
	     option->kind != SM_OBJ_NUMBER))
		return false;

	name = option_name(option);
	ui_name = option_ui_name(option);
	if (!name || !name[0] || !ui_name || !ui_name[0] ||
	    !option_flags_are_known(option_flags(option)))
		return false;
	...
	if (option->kind == SM_OBJ_NUMBER &&
	    (option->sm_number.min > number_max(option) || !option->sm_number.step ||
	     option->sm_number.display_flags & ~CFR_NUM_OPT_DISPFLAG_HEX))
		return false;

	return cfr_settings_value_is_valid(option, option_default_value(option)) &&
		dependency_is_well_formed(option);
}

static bool object_is_well_formed(const struct sm_object *object)
{
	...
	switch (object->kind) {
	case SM_OBJ_NONE:
		return dependency_is_well_formed(object);
	case SM_OBJ_ENUM:
	case SM_OBJ_NUMBER:
	case SM_OBJ_BOOL:
		return numeric_option_is_well_formed(object);
	case SM_OBJ_VARCHAR:
		return object->sm_varchar.opt_name && object->sm_varchar.opt_name[0] &&
			object->sm_varchar.ui_name && object->sm_varchar.ui_name[0] &&
			object->sm_varchar.default_value &&
			option_flags_are_known(object->sm_varchar.flags) &&
			dependency_is_well_formed(object);
	case SM_OBJ_COMMENT:
		return object->sm_comment.ui_name && object->sm_comment.ui_name[0] &&
			option_flags_are_known(object->sm_comment.flags) &&
			dependency_is_well_formed(object);
	case SM_OBJ_FORM:
		return form_is_well_formed(object);
	default:
		return false;
	}
}
...
static bool entry_is_well_formed(const struct cfr_settings_policy *entry)
{
	...
	if (!numeric_option_is_well_formed(option))
		return false;

	flags = option_flags(option);
	if (flags & CFR_OPTFLAG_VOLATILE)
		return false;
...
static bool record_object(const struct sm_object *object)
{
	if (!tree_scratch.collect_objects)
		return true;

	for (size_t i = 0; i < num_tree_objects; i++)
		if (tree_objects[i] == object)
			return false;

	if (num_tree_objects >= ARRAY_SIZE(tree_objects))
		return false;
	tree_objects[num_tree_objects++] = object;
	return true;
}
...
static bool walk_root(const struct sm_object *root)
{
	if (++tree_scratch.objects_seen > CFR_SETTINGS_MAX_TREE_OBJECTS ||
	    !form_is_well_formed(root) || !record_object(root))
		return false;
	...
	while (tree_scratch.depth) {
		...
		if (++tree_scratch.objects_seen > CFR_SETTINGS_MAX_TREE_OBJECTS ||
		    !object_is_well_formed(object) || !record_object(object))
			return false;
...
static size_t find_tree_object(const struct sm_object *object)
{
	for (size_t i = 0; i < num_tree_objects; i++)
		if (tree_objects[i] == object)
			return i;
...
static bool form_tree_is_well_formed(const struct sm_object *const *forms,
				     size_t num_forms)
{
	memset(&tree_scratch, 0, sizeof(tree_scratch));
	num_tree_objects = 0;
	tree_scratch.collect_objects = true;

	for (size_t i = 0; i < num_forms; i++)
		if (!walk_root(forms[i]))
			return false;

	memset(tree_dependency_states, TREE_DEPENDENCY_UNSEEN,
	       sizeof(tree_dependency_states));
	for (size_t start = 0; start < num_tree_objects; start++) {
		size_t depth = 0;

		if (tree_dependency_states[start] == TREE_DEPENDENCY_DONE)
			continue;
		tree_stack_object[0] = start;
		tree_stack_next[0] = 0;
		tree_dependency_states[start] = TREE_DEPENDENCY_VISITING;

		while (true) {
			const size_t current = tree_stack_object[depth];
			const struct target_path *path =
				find_target_path(tree_objects[current], forms, num_forms);
			size_t dependency;

			if (path->malformed || path->occurrences != 1)
				return false;
			if (tree_stack_next[depth] >= path->dependencies.num_entries) {
				tree_dependency_states[current] = TREE_DEPENDENCY_DONE;
				if (!depth)
					break;
				depth--;
				continue;
			...
			dependency = find_tree_object(
				path->dependencies.entries[tree_stack_next[depth]++].option);
			if (dependency == SIZE_MAX)
				return false;
			if (tree_dependency_states[dependency] == TREE_DEPENDENCY_DONE)
				continue;
			if (tree_dependency_states[dependency] == TREE_DEPENDENCY_VISITING)
				return false;
			if (++depth >= ARRAY_SIZE(tree_stack_object))
				return false;

			tree_stack_object[depth] = dependency;
			tree_stack_next[depth] = 0;
			tree_dependency_states[dependency] = TREE_DEPENDENCY_VISITING;
		}
	}
				  ...
				  size_t num_forms)
{
	...
	for (size_t i = 0; i < num_entries; i++) {
		...
		if (!entry_is_well_formed(&policy[i]))
			return false;
		name = option_name(policy[i].option);
...
static const struct cfr_settings_policy *get_policy(size_t *num_entries)
{
	...
	size_t current_form_entries;
	const struct cfr_settings_policy *policy =
		mainboard_cfr_settings_policy(&current_entries);
	const struct sm_object *const *forms =
		mainboard_cfr_settings_forms(&current_form_entries);

	if (validation_state == POLICY_NOT_VALIDATED) {
		validated_policy = policy;
		validated_policy_entries = current_entries;
		validated_forms = forms;
		validated_form_entries = current_form_entries;
		validation_state = policy_is_well_formed(policy, current_entries, forms,
						 current_form_entries) ?
			POLICY_VALID : POLICY_INVALID;
	}

	if (validation_state != POLICY_VALID || policy != validated_policy ||
	    current_entries != validated_policy_entries || forms != validated_forms ||
	    current_form_entries != validated_form_entries) {
		*num_entries = 0;
		return NULL;
	}
...
const struct sm_object *const *cfr_settings_policy_forms(size_t *num_forms)
{
	...
	if (!get_policy(&num_entries)) {
		*num_forms = 0;
		return NULL;
	}

	*num_forms = validated_form_entries;
					 ...
					 cfr_settings_read_fn read_option)
{
	...
	size_t num_forms;
	const struct cfr_settings_policy *policy = get_policy(&num_entries);
	const struct sm_object *const *forms = cfr_settings_policy_forms(&num_forms);
	const struct cfr_settings_policy *validated_entry;

	if (!entry || !read_option || !policy || !forms)
		return false;
