...
static size_t fudan_protected_size(const struct spi_flash *flash, u8 bp)
{
	if (bp < 3)
		return 0;
	if (bp == 7)
			     ...
			     bool *sr2_valid)
{
	...
	*sr2_valid = false;
	ret = spi_flash_status(flash, sr1);

	if (ret)
		return ret;

	ret = spi_flash_read_status(flash, CMD_FM25_RDSR2, sr2);
	if (!ret) {
		*sr2_valid = true;
		return 0;
	}

	/* Hardware-sequenced controllers may expose WRSR but not RDSR2. */
...
static void fudan_bp_to_region(const struct spi_flash *flash, u8 bp, bool tb,
			       bool cmp, struct region *region)
{
	size_t protected_size = fudan_protected_size(flash, bp);
					     ...
					     bool exact)
{
	...
	ret = fudan_read_status(flash, &sr1, &sr2, &sr2_valid);
	if (ret)
		return ret;
	if (!sr2_valid)
		return -1;
	...
	fudan_bp_to_region(flash, (sr1 & FM25_SR1_BP_MASK) >> FM25_SR1_BP_SHIFT,
			   !!(sr1 & FM25_SR1_TB),
