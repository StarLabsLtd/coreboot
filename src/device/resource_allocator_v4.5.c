				  ...
				  struct memranges *const ranges)
{
	...
	if (CONFIG(RESOURCE_ALLOCATION_COMPAT)) {
		struct range_entry *r, *biggest = NULL;

		/* Don't allow allocations above 4G. */
		memranges_create_hole(ranges, POWER_OF_2(32), UINT64_MAX - POWER_OF_2(32) + 1);

		/* Set all but the biggest range's tag to 0. */
		memranges_each_entry(r, ranges) {
			if (!biggest) {
				biggest = r;
				continue;
			...
			if (range_entry_size(r) > range_entry_size(biggest)) {
				range_entry_update_tag(biggest, 0);
				biggest = r;
			} else {
				range_entry_update_tag(r, 0);
			}
		}
	}
					...
					const unsigned long type)
{
	...
	const struct resource *const domain_res = find_domain_resource(domain, type);
	if (!domain_res)
		return;

	setup_resource_ranges(domain, domain_res, type, &ranges);
