...
{
	unsigned int me_state_counter = get_uint_option("me_state_counter", UINT_MAX);

	if (me_state_counter != UINT_MAX) {
		printk(BIOS_DEBUG, "CMOS: me_state_counter = %u\n", me_state_counter);
		/* Avoid boot loops by only trying a state change 3 times */
		if (me_state_counter < ME_DISABLE_ATTEMPTS) {
			me_state_counter++;
			set_uint_option("me_state_counter", me_state_counter);
...
static void cse_set_state(struct device *dev)
{
	...
	struct state_reply reply;

	size_t reply_size;

	/*
	 * We only take action if the me_state doesn't match the me working state
	 */
	...
	if (me_state && !soft_temp_disable) {
		...
		send = heci_send_receive(&me_disable, sizeof(me_disable), &reply, &reply_size,
									  HECI_MKHI_ADDR);
	} else if (!me_state && soft_temp_disable) {
		/* me_state should be enabled, but it's disabled */
		printk(BIOS_DEBUG, "ME needs to be enabled.\n");
		send = heci_send_receive(&me_enable, sizeof(me_enable), &reply, &reply_size,
									    HECI_MKHI_ADDR);
	} else {
		printk(BIOS_DEBUG, "ME is %s.\n", me_state ? "disabled" : "enabled");
		unsigned int me_state_counter = get_uint_option("me_state_counter",
								 UINT_MAX);
	 ...
	 * version of the Intel ME won't successfully receive the message unless reset
	 * twice.
	 */
	if (send || !result)
		me_reset_with_count();
