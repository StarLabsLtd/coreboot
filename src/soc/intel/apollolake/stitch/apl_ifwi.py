...
def create_ifwi(args):
	...
	if args.private_key:
		...
		bios = create_bios(read_file(args.ibbl), read_file(args.ibb),
				   read_file(args.obb), args.private_key,
				   public_key, exponent)
		if args.smip:
		...
		oem_key = create_oem_key_manifest(args.private_key, public_key,
						  exponent, public_hash)
	else:
		bios = read_file(args.bios)
		smip = read_file(args.smip)
		oem_key = read_file(args.oem_key)
	...
	if args.uep:
		...
		raise ValueError("UEP input or private key is required")
...
def main(argv):
	...
	parser.add_argument("--ibbl")
	parser.add_argument("--ibb")
	...
	if args.private_key:
		for arg_name in ("ibbl", "ibb", "obb"):
			if getattr(args, arg_name) is None:
				parser.error(f"--{arg_name} is required with --private-key")
	else:
		for arg_name in ("bios", "oem_key", "smip"):
			if getattr(args, arg_name) is None:
				parser.error(f"--{arg_name.replace('_', '-')} is required without "
					     "--private-key")
