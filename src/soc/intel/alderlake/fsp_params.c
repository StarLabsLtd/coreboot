				   ...
				   uint32_t index)
{
	...
	if (!CONFIG(PCIEXP_CLK_PM) || !CONFIG(PCIEXP_ASPM)) {
		s_cfg->PcieRpAspm[index] = 0;
		s_cfg->PcieRpL1Substates[index] = 0;
		return;
	}

	/* Use the default "auto" unless overwritten */
	if (rp_cfg->pcie_rp_aspm)
		s_cfg->PcieRpAspm[index] = rp_cfg->pcie_rp_aspm;
	...
	/* Use the default "max" unless overwritten */
	if (rp_cfg->PcieRpL1Substates)
		s_cfg->PcieRpL1Substates[index] = rp_cfg->PcieRpL1Substates;
}
				       ...
				       uint32_t index)
{
	...
	/* Use the default "auto" unless overwritten */
	if (rp_cfg->pcie_rp_aspm)
		s_cfg->CpuPcieRpAspm[index] = rp_cfg->pcie_rp_aspm;
	...
	/* Use the default "max" unless overwritten */
	if (rp_cfg->PcieRpL1Substates)
		s_cfg->CpuPcieRpL1Substates[index] = rp_cfg->PcieRpL1Substates;
}
