...
#if !ENV_SMM && !ENV_TEST
#error "Payload-MM authenticated-variable QEMU pflash backend is SMM-only"
#endif

#define QEMU_PFLASH_BACKEND_REVISION 1U
...
static bool context_valid(const void *opaque)
{
	...
	if (!context || context->revision != QEMU_PFLASH_BACKEND_REVISION ||
...
static bool span_valid(uint32_t offset, size_t size, uint32_t *absolute)
{
	...
	*absolute = backend.policy.store_offset + offset;
	return *absolute <= region_device_sz(backend.sealed.root) &&
		size <= region_device_sz(backend.sealed.root) - *absolute;
}
	...
	uint64_t *generation)
{
	...
	*generation = 0;
	if (boot_device_rw() != backend.sealed.root ||
	    region_device_sz(backend.sealed.root) < backend.policy.store_offset ||
	    backend.policy.store_size >
		region_device_sz(backend.sealed.root) - backend.policy.store_offset ||
	    backend.policy.erase_size != 4096U ||
	    qemu_pflash_lease_begin(backend.sealed.root,
		&backend.lease)) {
		poison();
	...
	uint32_t offset, void *buffer, size_t size, size_t *completed)
{
	...
	result = qemu_pflash_lease_read(backend.sealed.root,
		&backend.lease, absolute, buffer, size);
	...
	uint32_t offset, const void *buffer, size_t size)
{
	...
	result = qemu_pflash_lease_program(backend.sealed.root,
		&backend.lease, absolute, buffer, size);
	if (result || !context_valid(opaque)) {
		poison();
		return PAYLOAD_MM_AUTHVAR_MEDIA_DEVICE_ERROR;
	...
	uint32_t offset, size_t size)
{
	...
	result = qemu_pflash_lease_erase(backend.sealed.root,
...
static enum payload_mm_authvar_media_result sync_media(const void *opaque)
{
	...
	result = qemu_pflash_lease_sync(backend.sealed.root,
		&backend.lease);
...
enum cb_err payload_mm_authvar_qemu_pflash_install(void)
{
	...
	if (!__atomic_compare_exchange_n(&backend.install_attempted, &expected, 1,
		...
		return CB_ERR;
	root = boot_device_rw();
	if (!root || contract.boot_media_size != region_device_sz(root) ||
	    ...
	    contract.erase_size != 4096U ||
	    contract.store_offset > region_device_sz(root) ||
	    contract.store_size >
		region_device_sz(root) - contract.store_offset)
		return CB_ERR;
	...
	backend.context = (struct backend_context) {
		.revision = QEMU_PFLASH_BACKEND_REVISION,
