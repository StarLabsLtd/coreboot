...
static bool smbios_dimm_uses_onboard_fallbacks(const struct dimm_info *dimm,
					       const struct spd_info *info)
{
	if (info->form_factor == MEMORY_FORMFACTOR_CHIP ||
					   ...
					   bool use_onboard_fallbacks)
{
	...
	if (use_onboard_fallbacks &&
	    !dimm->serial[0] && !dimm->serial[1] &&
	    !dimm->serial[2] && !dimm->serial[3]) {
		snprintf(serial, sizeof(serial), "OnboardS%uC%uCH%uD%u", dimm->soc_num,
					 ...
					 int type16_handle)
{
	...
	bool use_onboard_fallbacks = smbios_dimm_uses_onboard_fallbacks(dimm, &info);
...
// If DDR5 memory is used, the definition of the bus_width field in the SPD spec is different from that of DDR4
	...
	smbios_fill_dimm_serial_number(dimm, t, use_onboard_fallbacks);
	...
	smbios_fill_dimm_part_number(dimm, t, use_onboard_fallbacks);
