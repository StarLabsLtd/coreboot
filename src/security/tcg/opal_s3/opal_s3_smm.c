...
static void *find_save_state(const struct smm_save_state_ops *save_state_ops, int cmd)
{
	...
	/* Find the CPU that issued the synchronous APMC IO write. */
	for (node = 0; node < CONFIG_MAX_CPUS; node++) {
		...
		reg_al = (u8)save_state_ops->get_reg(state, RAX);
		if (reg_al != cmd)
			continue;
...
int opal_s3_smi_apmc(u8 apmc)
{
	...
	case APM_CNT_OPAL_S3_UNLOCK:
		...
		if (save_state_ops) {
			state = find_save_state(save_state_ops, apmc);
			if (state)
				save_state_ops->set_reg(state, RAX, unlock_rc);
		}
		return 1;
	...
	rax = save_state_ops->get_reg(state, RAX);
	rbx = save_state_ops->get_reg(state, RBX);
	subcmd = (rax >> 8) & 0xff;
